...
if [ $? -eq 0 ]; then # Exit with OK
    ReadPackage=$(pm list packages -3 | sed -e 's/package://g' | sed "${ReadPackagename}!d")
        ...
        2)
            ...
            cd "/data/GBackup/"
            (   
                echo "Running tar -pczf /sdcard/GBackup/PersonalBkp/${ReadPackage}_GBackup.tar.gz
            Please wait....."
...
if [[ -d /sdcard/GBackup/PersonalBkp ]]; then
    ...
    BACKTITLE=$(gecpc "Made By Jaxparrow | GUI by Xtr" "_")
    TITLE="++++ Backups ++++"
    MENU="Please Put your files in ${YELLOW}GBackup/PersonalBkp${RC} of Main Storage"
        ...
        case $file in
                $string)
                    ...
                    unzip "${file}" -d "/data/GRestore/"
                                ...
                                (   
                echo "Running tar -xpf $file -C /data/GRestore
            Please wait....."
                tar -xpf "$file" -C "/data/GRestore"
            ) | dialog --progressbox "Backup in progress" 7 45

                fi
            dialog --msgbox "$TASK Restoring Backup.. Please Wait
            $CAUTION Do not switch to GUI while restoring" 7 45
            cd "/data/GRestore/"
